...
NIVALIS provides a seamless, one-click solution that throttles CPU frequency and caps GPU wattage to **reduce PC heat generation by up to 45%** while keeping the system 100% usable—all without touching your display settings or color profiles.
...
### ⚡ Performance Mode
- Instantly restores **100% CPU max frequency**, enables **Active fan cooling**, and removes GPU wattage caps (restoring full 125W factory power).

### 📌 Automatic Windows Startup
- Automatically registers in the Windows Registry to start silently with Windows into the System Tray near the clock every time your PC turns on.
...
- **Power Schemes**: Switches between **Power Saver** (`a1841308-3541-4fab-bc81-f71556f20b4a`) and saved original scheme / **High Performance** (`8c5e7fda-e8bf-4a96-9a85-a6e23a8c635c`).
...
### 3. Real-Time Math-Based Audio Synthesizer
...
### 4. Windows Registry Auto-Start Integration
...
3. Use the interface to toggle between **Eco Mode** and **Normal Mode**.
